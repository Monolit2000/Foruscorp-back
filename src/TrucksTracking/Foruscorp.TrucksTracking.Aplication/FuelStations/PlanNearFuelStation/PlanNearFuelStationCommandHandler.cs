...
{
    public record PlanNearFuelStationCommand(Guid FuelStationId, Guid RouteId, Guid TruckId, string Address, double NearDistance, double Longitude, double Latitude) : IRequest;
        ...
        ITruckTrackingContext tuckTrackingContext) : IRequestHandler<PlanNearFuelStationCommand>
    {
        public async Task Handle(PlanNearFuelStationCommand request, CancellationToken cancellationToken)
        {
            ...
            var fuelStationPlan = NearFuelStationPlan.Create(
                request.FuelStationId, truckTracker.TruckId, request.RouteId, request.Longitude, request.Latitude, request.Address, request.NearDistance);
